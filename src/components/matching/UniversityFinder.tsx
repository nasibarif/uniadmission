...
export const UniversityFinder: React.FC = () => {
  ...
  const [categoryFilter, setCategoryFilter] = useState<'All' | 'Reach' | 'Target' | 'Safe'>('All');
  ...
  const safeCount = universities.filter(u => u.category === 'Safe').length;

  // Filter & sort
  let filtered = universities.filter(uni => {
    if (categoryFilter !== 'All' && uni.category !== categoryFilter) return false;
    ...
    <div className="space-y-6 pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 border border-blue-100 text-blue-700 text-xs font-semibold mb-2">
              ...
              <span>Smart Admission Matching Engine</span>
            </div>
            <h2 className="text-2xl font-bold text-slate-900">
              Recommended Application Portfolio
            </h2>
            <p className="text-xs text-slate-600 mt-1 max-w-xl">
              Universities classified based on your GPA ({profile.academic.rawGpaText}), {profile.standardizedTests.englishTest.type} ({profile.standardizedTests.englishTest.overallScore || '7.0'}), and budget (${profile.financial.maxYearlyBudgetUSD}/yr).
            </p>
          </div>

          {/* 3:5:3 Portfolio Stats */}
          <div className="flex items-center gap-3 bg-slate-50 p-2.5 rounded-xl border border-slate-200 shrink-0">
            <button
              ...
              className={`text-center px-3 py-1.5 rounded-lg transition ${categoryFilter === 'Reach' ? 'bg-purple-100' : 'hover:bg-slate-200/60'}`}
            >
              <div className="text-lg font-bold text-purple-700">{reachCount}</div>
              <div className="text-[10px] uppercase font-bold text-purple-600">Reach</div>
            </button>
            <span className="text-slate-300">•</span>
            <button
              onClick={() => setCategoryFilter('Target')}
              className={`text-center px-3 py-1.5 rounded-lg transition ${categoryFilter === 'Target' ? 'bg-blue-100' : 'hover:bg-slate-200/60'}`}
            >
              <div className="text-lg font-bold text-blue-700">{targetCount}</div>
              <div className="text-[10px] uppercase font-bold text-blue-600">Target</div>
            </button>
            <span className="text-slate-300">•</span>
            <button
              onClick={() => setCategoryFilter('Safe')}
              className={`text-center px-3 py-1.5 rounded-lg transition ${categoryFilter === 'Safe' ? 'bg-emerald-100' : 'hover:bg-slate-200/60'}`}
            >
              <div className="text-lg font-bold text-emerald-700">{safeCount}</div>
              <div className="text-[10px] uppercase font-bold text-emerald-600">Safe</div>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-xs flex flex-col md:flex-row gap-3 items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto w-full md:w-auto">
          {(['All', 'Reach', 'Target', 'Safe'] as const).map((cat) => {
            ...
            return (
              ...
              >
                {cat === 'All' ? 'All Matches' : cat === 'Reach' ? '🟣 Reach' : cat === 'Target' ? '🔵 Target' : '🟢 Safe'}
              </button>
            );
          })}
        </div>
        ...
        {displayedUnis.map((uni) => {
          ...
          const probBadge = 
            uni.admissionProbability === 'High' 
              ...
              : uni.admissionProbability === 'Moderate' 
          ...
          return (
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-500">
                    {uni.city}, {uni.country} • World Rank <strong className="text-slate-700">#{uni.rankingWorld}</strong> • Acceptance Rate: <strong className="text-slate-700">{(uni.acceptanceRate * 100).toFixed(0)}%</strong>
                  </p>

                  <div className="flex flex-wrap gap-2 text-xs">
                    <span className={`px-2 py-0.5 rounded-md border font-semibold text-[11px] ${probBadge}`}>
                      Admission Probability: <strong>{uni.admissionProbability}</strong>
                    </span>
                    <span className="px-2 py-0.5 rounded-md border border-slate-200 bg-slate-50 text-slate-700 font-semibold text-[11px]">
                      Scholarship Potential: <strong>{uni.scholarshipProbability}</strong>
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row md:flex-col items-start md:items-end justify-between gap-3 shrink-0">
                  <div className="text-left md:text-right">
                    ...
                    <span className="text-[10px] text-slate-400 line-through">Gross: ${uni.averageAnnualTuitionUSD.toLocaleString()}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <a
                      ...
                      title="Open Official Admission Portal"
                    >
              ...
              {uni.whyMatch && uni.whyMatch.length > 0 && (
                <div className="pt-3 border-t border-slate-100 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Why you qualify:</span>
        ...
        <div className="p-6 rounded-2xl bg-amber-50/60 border border-amber-200 text-center space-y-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600 max-w-md mx-auto mt-1">
              You are currently viewing a limited preview (3 of {universities.length} matched universities). Switch to the Explorer or Complete tier to unlock full discovery.
            </p>
          </div>
          <button
